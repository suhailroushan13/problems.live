...
export async function toggleCommentHelpful(
  ...
  try {
    ...
    let active: boolean;
    try {
      await CommentVote.create({ commentId: cid, userId: uid });
      active = true;
    } catch (error) {
      if (!isDuplicateKeyError(error)) throw error;
      await CommentVote.deleteOne({ commentId: cid, userId: uid }).exec();
      active = false;
    }
    ...
    await awardReputation(
      ...
      active ? REPUTATION.COMMENT_HELPFUL : -REPUTATION.COMMENT_HELPFUL,
      { key: "helpfulVotes", delta }
    );
